...
9. При заведении нового репозитория первый коммит должен иметь сообщение
   ...
   - AUTHORS - содержит список авторов (файл должен присутствовать в открытых
     проектах, выложеных на GitHub)
...
1. Перед фиксом бага обязательно написание теста, воспроизводящего этот баг.
   ...
   проджект-/тимлида. Желательно, чтобы тест повторял баг, в таком виде,
   как он зарепорчен. Рекомендуется по возможности дополнительно писать 
   более "чистые" тесты, чтобы выявить первопричину проблемы.
...
3. После фикса, рекомендуется переставить коммиты местами, чтобы не ломать
   `git bisect`: коммит с исправлением должен идти первым, далее коммит с тестом
   В коммитлоге у коммита с тестом надо оставить соотвествующий комментарий,
   что этот тест относится к предыдущему коммиту с фиксом.
